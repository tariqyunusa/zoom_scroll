...
const ThreeScene = () => {
    ...
    useEffect(() => {
        ...
        renderer.setClearColor('#FAF9F6', 1); // Set off-white background

        if (mountRef.current) {
            mountRef.current.appendChild(renderer.domElement);
        }

        // Create Meshes with Preloaded Textures
        const meshesArray = [];
        textures.forEach((texture, index) => {
            const initialPosY = index * 4 - (4 * (textures.length - 1)) / 2; // Evenly space meshes
        ...
        scroller.on((event) => {
            scrollTarget.current += event.deltaY * 0.1; // Adjust scroll sensitivity
        });
        ...
        const animate = () => {
            ...
            meshesArray.forEach((mesh, index) => {
                mesh.position.y = initialPos.current[index] - scrollCurrent.current;
